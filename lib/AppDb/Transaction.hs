module AppDb.Transaction where

import           AppDb.Prelude
import           Hasql.Transaction
import qualified AppDb.Statement               as Statement
...
register
  :: Text -> ByteString -> Text -> Maybe Text -> Transaction (Bool, Int32)
register email password name phone = session Write Serializable $ do
  ...
  case possibleExistingId of
    ...
    Nothing         -> do
      newId <- Session.statement (email, password, name, phone)
...
-- This is an example of how you can compose transactions sequentially.
register'
  :: Text -> ByteString -> Text -> Maybe Text -> Transaction (Bool, Int32)
register' email password name phone = fromMaybeS
  (fmap (\newId -> (True, newId)) (insertUser email password name phone))
  (fmap (fmap (\existingId -> (False, existingId))) (findUserByEmail email))
...
insertUser email password name phone = statement
  Write
  Serializable
  Statement.insertUser
