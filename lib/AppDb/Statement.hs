...
findUserByEmail = Statement "select id from user where email = $1"
...
insertUser =
  let
    ...
    \values ($1, $2, $3, $4) \
    \returning id"
    encoder = contrazip4 (E.param (E.nonNullable E.text))
...
authenticateUser =
  let
    sql     = "select password = $2, id from user where email = $1"
    encoder = contrazip2 (E.param (E.nonNullable E.text))
                         (E.param (E.nonNullable E.bytea))
    decoder =
      D.rowMaybe $ (,) <$> D.column (D.nonNullable D.bool) <*> D.column
        (D.nonNullable D.int4)
  in
    Statement sql encoder decoder True
...
insertNotification = Statement
  "insert into notification (user, message, read) \
    \values ($1, $2, 'false') \
    \returning id"
  (contrazip2 (E.param (E.nonNullable E.int4)) (E.param (E.nonNullable E.text)))
